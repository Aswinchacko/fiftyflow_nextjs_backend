...
async function handlePost(request) {
  ...
  const { validated, error: valErr } = validateBody(loginSchema, body);
  ...
  try {
    ...
    if (!user || !(await authService.verifyPassword(password, user.passwordHash))) {
      return NextResponse.json({ error: errors.INVALID_CREDENTIALS, code: 'INVALID_CREDENTIALS' }, { status: 401 });
    }
